...
- `src/lib/geofence.ts` — leave-gym auto-end (native only). Wraps `@capacitor-community/background-geolocation`; `captureLocation` (one-shot fix for "set gym location"), `startGeofence(onLeave)`/`stopGeofence` (foreground-service watcher; fires once after you're confirmed at the gym then `>radius` for 2 min). Gym stored in settings `gymLat`/`gymLng`/`gymRadiusM`/`autoEndOnLeave`. Wired in `Today` (starts per non-custom session, auto-finishes on leave). Needs the location perms in AndroidManifest.
