...
_Last verified: 2026-08-03 @ 1.55.0 — #5 interval-note (`intervalSummary` → auto-appended note on custom sessions). Prior @1.54.0: backlog batch — break-skip sound, PR badge + `playPr` (`--volt`), break recording (`WorkoutBreak`/`closeCurrentBreak` → History `⏸` stats + amber RunMap markers), active-exercise cue. @1c82fe0 slimmed the 48 KB CODE-MAP into this thin index._
...
- **Not built yet:** the **GO** half of the `--volt` moments (weekly-goal-hit celebration — the **PR** half shipped 1.54.0, DECLINED for now); #8 Wear OS (DEFERRED — separate native module). #5 auto-note break intervals shipped 1.55.0. P4 copy polish is effectively done (History renders "March 2021"; only a heatmap tooltip shows raw `YYYY-MM`). (Google Sheets read+write-back, in-app template/exercise editing, and per-lift charts ARE built.)
